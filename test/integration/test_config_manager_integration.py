...
class TestConfigManagerIntegration:
    ...
    def test_large_config_file_performance(self, temp_config_file):
        ...
        large_config = {
            ...
            "native_asset": "ETH",
            "networks": {
                f"NETWORK_{i}": f"https://network-{i}.example.com" 
                for i in range(100)
            },
            "assets": {
                f"ASSET_{i}": {
...
"""
    ...
    def test_config_file_with_special_characters(self, temp_config_file):
        ...
        with patch_config_path(temp_config_file):
            ...
            assert config_manager.get_rpc_url("TEST") == "https://test.example.com/path?param=value&other=123"
            
            # Test other functionality still works
            assert config_manager.get_current_network() == "TEST"
            assert sorted(list(config_manager.get_networks())) == sorted(["TEST", "LOCAL", "ETHEREUM"])
    ...
    def test_multiple_config_manager_instances(self, temp_config_file):
        ...
        with patch_config_path(temp_config_file):
            ...
            assert list(config_manager1.get_networks()) == list(config_manager2.get_networks())
